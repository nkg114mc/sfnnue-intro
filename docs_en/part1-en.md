...
Stockfish is an open source chess engine program. Since two of the original developers were from Norway and Italy, the developers named their program Stockfish in honor of the historic cod trade between the two countries. In the past ten years, a group of enthusiasts have continuously maintained and improved this chess program, making it one of the most powerful chess programs. Stockfish has brought traditional computer chess techniques to the extreme, including but not limited to alpha-beta search, various pruning techniques, and valuation functions with manually adjusted parameters, etc. At least before 2017, Stockfish was almost firmly on the first place in the chess power rankings of major computer chess programs.
...
However, in 2017, Deep Mind published a paper on Alpha Zero , and proposed that the method of combining Monte-Calo Tree Search (MCTS) with neural networks can still reach or even surpass Stockfish's existing chess power. So in 2018, Leela Chess Zero (Lc0) was born. According to the method in the Alpha Zero paper, another group of enthusiasts finally reproduced the results in the Alpha Zero paper after nearly a year of development and parameter training. As of the end of 2019, on the CCRL (Computer Chess Rating List), Lc0 once surpassed Stockfish and became the first chess program in the neural network era to achieve the world's top level by relying on pure reinforcement learning methods.
...
For now, people can't help but ask, in the field of computer science, can the traditional alpha-beta-based search method be completely abandoned? The era of deep reinforcement learning has completely crushed all traditional methods? However, the reality is not that simple. Although the method of Lc0 has achieved good results, due to the use of unsupervised (or environment feedback as supervision) strategy, the computational cost of its training is also staggering. Is there any way to improve the efficiency of machine learning algorithms by using existing techniques in traditional methods? Or is it possible to combine traditional search methods with neural networks and learn from each other? Can supervised learning be introduced to improve training speed?
...
An obvious direction of exploration can be realized by most people intuitively: 



 seems to be what people can think of : the program based on alpha-beta search needs to call the evaluation function to output an evaluation score for the current situation, that is, the probability of a certain party winning. Whether it is accurate or not directly determines whether the orientation of the search is correct. Since the representation ability of the neural network is so powerful, why not replace the traditional handwritten evaluation function with a neural network model, and then use deep learning methods to train this model to improve the accuracy of the evaluation function?
...
In fact, the matter of "formalizing the valuation function into a machine learning model and using machine learning techniques to train it" has been done a long time ago. For example, Temporal Difference Learning , which was proposed in the 1950s , and TD-Leaf (lambda) , which was further improved by Arthur Samuel, Don Beal and Martin Smith in 1997 , were successfully applied to KnightCap and CilkChess. However, people at that time did not know that introducing a nonlinear model could improve the representation ability of the model, and the limited computing power of the computer did not support the use of larger training data.
...
In 2015, Matthew Lai, a master student from Imperial College of Technology, improved the training method of TD-Leaf (lambda) in the national elephant, replaced the original linear model with a simple three-layer neural network, and released the experimental national elephant. Like the program Giraffe . After about a week of training, Giraffe has progressed from playing chess at random to 2400ELO (the level of a "chess master" candidate under the FIDE standard), which is a huge improvement. It has to be said that in 2015, when PyTorch was not yet released, in order to use Torch in training, Matthew first wrote the network itself and some functions needed for training into Lua programs, and then encapsulated these Lua programs with the C++ library that comes with Torch And glue it together with the C++ code of your own national elephant program. It can be said that Giraffe can achieve such results, and the efforts made by Matthew can be described as "epic and tearful". However, Giraffe is only the first step to explore this method in the era of deep learning, and it is still too far away from challenging the world-class national elephant program.
...
So far, people have found that: in order to use the neural network evaluation function in alpha-beta search, the following three problems have to be solved:
...
In May 2020, things took a turn for the better. A post about Stockfish-NNUE appeared on the chess programming forum Talkchess. A poster with the ID Nodchip introduced NNUE technology into Guoxiang for the first time. At the end of July, the source code of Stockfish-NNUE was officially released on Github . After testing and improvement, Stockfish-NNUE has reached or even surpassed Stockfish using traditional evaluation functions, and was officially merged into the main branch of Stockfish at the beginning of September 2020, becoming a part of Stockfish 12.

The full name of NNUE is Efficiently Updatable Neural Network (fast update neural network). It was originally proposed by Japanese shogi programmer Nasu Yu in 2018 and applied to shogi programs. Nodchip is also a member of this shogi program development team. Specifically, Stockfish-NNUE has made the following contributions:
...
1. A binary and high-dimensional representation of the board is designed, and only incremental updates are made during the game or search. Unlike previous neural network chess programs, the NNUE board representation is very simple, that is, the combination of the square where the king is located and the squares where other chess pieces are located. It is precisely because of this feature that the chessboard representation of NNUE can be updated incrementally, because the pieces or grids that can be affected by each move are limited;
1. A shallow (4-layer) neural network is used as the evaluation function model. All parameters of the network are integers, and use advanced instructions in Intel CPU to optimize the operational efficiency of matrix addition and multiplication;
1. A supervised learning evaluation function training method is proposed, and two kinds of target values ​​are used. Generally speaking, supervised learning is used to train the evaluation function, and the target value of the training data generally comes from the value returned by the search algorithm after searching a certain depth using the handwritten evaluation function. NNUE's training data naturally also uses this target value, but in addition, NNUE also uses the final result of the self-play chess game (-1, 0, +1 corresponds to loss, draw, and win) as the second target value, Finally, the weighted average of the two goals is taken as the final goal. The weighted weight is controlled by the parameter lambda.
1. A parallel data generation algorithm is designed, using the original Stockfish to automatically and efficiently generate training data.

There is no doubt that Stockfish-NNUE is by-far the most successful attempt at the method of "alpha-beta search + neural network evaluation" after Giraffe. In the three questions mentioned above, Stockfish-NNUE provided an answer to each of them. 


This gives programmers who use alpha-beta search a glimmer of hope, especially when Lc0 has been firmly in the first place for nearly a year, the application of NNUE technology makes Stockfish become the "No. 1" of Lc0 again one of the challengers. 

<!-- After that, I will introduce the application of NNUE in Stockfish in more detail several times. -->

| ![title](./img/p1-1.png) |
| :---:       |
| <em>CCRL Blitz Ranking Top 10 (by September 12, 2020)</em> |
